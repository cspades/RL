...
case "${TASK}" in
  clevr)
    ...
    ;;
  vstat)
    CONFIG="${CONFIG:-examples/configs/recipes/vlm/vlm_grpo-nemotron-omni-30ba3b-vstat-1n4g-megatron-single-controller-async.v1.yaml}"
    ...
    TASK_ENV="DATA_ROOT=${DATA_ROOT} NUM_DATA_ROWS=${NUM_DATA_ROWS} NUM_FRAMES=${NUM_FRAMES} TEMPORAL_PATCH_SIZE=${TEMPORAL_PATCH_SIZE} VIDEO_TARGET_PATCHES=${VIDEO_TARGET_PATCHES} MIN_GENERATION_TOKENS=${MIN_GENERATION_TOKENS} ENABLE_THINKING=${ENABLE_THINKING} PREPARE_VSTAT=${PREPARE_VSTAT}"
    TASK_OVERRIDES="\
