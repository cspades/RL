...
# NeMo-RL V1 twin of submit_super_vl_35_videoqa_vllm_v2_32n4g.sh.
# Model, data, topology, sampling, and media geometry match V2. V1-specific
# memory settings preserve the global training batch while avoiding GPU OOM.
...
# V1 retains more training memory than SingleController. Keep the global batch
# and full sequence capacities while using TP=4 to reduce per-GPU memory.
...
export RESULTS_DIR="${RESULTS_DIR:-${NEMORL}/workspace/results/super-vl-35-videoqa-vllm-v1}"
...
export VIDEO_TEACHER_WANDB_NAME="${VIDEO_TEACHER_WANDB_NAME:-super-vl-35-videoqa-vllm-v1}"
...
export JOB_NAME="${JOB_NAME:-super-vl-35-videoqa-vllm-v1-32n4g}"
